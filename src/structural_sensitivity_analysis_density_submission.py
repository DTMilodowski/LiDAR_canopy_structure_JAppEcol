...
#plot = 'B North'
plot = sys.argv[1]
sample_res = np.array([float(sys.argv[2])])
if plot == 'BNorth':
    plot = 'B North'
print(plot)
max_height = 80.
...
plot_width = 100.
#sample_res = np.array([10.])
keys = ['20m']
kappa = 0.70
n_iter = 250

area = 10.**4
target_pulse_density = np.array([2.5, 5., 10., 20., 25.])
keys_2 = ['2.5','5','10','20','25']
        ...
        for ss in range(0,sample_res.size):
            ...
            for pp in range(0,n_subplots):
                ...
                if np.sum(sp_pts[:,3]==1)>0: # check for returns within this column
                    ...
                    mh_wt_profile = LAD1.estimate_LAD_MacArthurHorn(weighted_return_profile,weighted_n_ground_returns,layer_thickness,kappa,zero_nodata=False)
                else:
                ...
                nodata_test = np.any(~np.isfinite(mh_wt_profile[2:]))
...
np.save("MH_wt_density_sensitivity_%s_%im_grid.npy" % (plot, int(sample_res)), PAD_profiles_MH_wt)
