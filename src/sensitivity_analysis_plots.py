...
"""
def plot_profile_sensitivity_resolution_single(figure_number,figure_name,heights,PAD_profiles_Belian,
                            PAD_profiles_BNorth,PAD_profiles_E):
    ## Belian
    ...
    for pp in range(0,len(sp)):
        skip_rad = False
        if pp<5:
            prof_sp = np.nanmean(PAD_profiles_Belian[pkeys[pp]]['25'],axis=1)
        elif pp<10:
            prof_sp = np.nanmean(PAD_profiles_E[pkeys[pp-5]]['25'],axis=1)
        else:
            prof_sp = np.nanmean(PAD_profiles_BNorth[pkeys[pp-10]]['25'],axis=1)

        prof_sp[~np.isfinite(prof_sp)]=np.nan
        prof = np.nanmedian(prof_sp,axis=0)
        perc = np.nanpercentile(prof_sp,[2.5,97.5],axis=0)
        sp[pp].fill_betweenx(heights[2:],perc[0][2:],perc[1][2:],color=colour[0],alpha=0.3)
        sp[pp].plot(prof[2:],heights[2:],'-',c=colour[0],linewidth=1.5)
...
"""
def plot_profile_sensitivity_density_single(figure_number,figure_name,heights,PAD_profiles_Belian,
                            PAD_profiles_BNorth,PAD_profiles_E,res_key='10m'):
    res_key = '20m'
    ...
    ax2a.annotate('a - 2.5 shots m$^{-2}$', xy=(0.05,0.95), xycoords='axes fraction',backgroundcolor='none',horizontalalignment='left', verticalalignment='top', fontsize=8)

    ax2b = plt.subplot2grid((3,5),(0,1),sharex=ax2a,sharey=ax2a)
    ax2b.annotate('b - 5 shots m$^{-2}$', xy=(0.05,0.95), xycoords='axes fraction',backgroundcolor='none',horizontalalignment='left', verticalalignment='top', fontsize=8)

    ax2c = plt.subplot2grid((3,5),(0,2),sharex=ax2a,sharey=ax2a)
    ax2c.annotate('c - 10 shots m$^{-2}$', xy=(0.05,0.95), xycoords='axes fraction',backgroundcolor='none',horizontalalignment='left', verticalalignment='top', fontsize=8)
    ...
    ax2f.annotate('f - 2.5 shots m$^{-2}$', xy=(0.05,0.95), xycoords='axes fraction',backgroundcolor='none',horizontalalignment='left', verticalalignment='top', fontsize=8)

    ax2g = plt.subplot2grid((3,5),(1,1),sharex=ax2a,sharey=ax2a)
    ax2g.annotate('g - 5 shots m$^{-2}$', xy=(0.05,0.95), xycoords='axes fraction',backgroundcolor='none',horizontalalignment='left', verticalalignment='top', fontsize=8)

    ax2h = plt.subplot2grid((3,5),(1,2),sharex=ax2a,sharey=ax2a)
    ax2h.annotate('h - 10 shots m$^{-2}$', xy=(0.05,0.95), xycoords='axes fraction',backgroundcolor='none',horizontalalignment='left', verticalalignment='top', fontsize=8)
    ...
    ax2k.annotate('k - 2.5 shots m$^{-2}$', xy=(0.05,0.95), xycoords='axes fraction',backgroundcolor='none',horizontalalignment='left', verticalalignment='top', fontsize=8)

    ax2l = plt.subplot2grid((3,5),(2,1),sharex=ax2a,sharey=ax2a)
    ax2l.annotate('l - 5 shots m$^{-2}$', xy=(0.05,0.95), xycoords='axes fraction',backgroundcolor='none',horizontalalignment='left', verticalalignment='top', fontsize=8)

    ax2m = plt.subplot2grid((3,5),(2,2),sharex=ax2a,sharey=ax2a)
    ax2m.annotate('m - 10 shots m$^{-2}$', xy=(0.05,0.95), xycoords='axes fraction',backgroundcolor='none',horizontalalignment='left', verticalalignment='top', fontsize=8)
    ...
    pkeys = ['2.5','5','10','20','25']
    for pp in range(0,len(sp)):
        if pp<5:
            prof_sp = np.nanmean(PAD_profiles_Belian[res_key][pkeys[pp]],axis=1)
        elif pp<10:
            prof_sp = np.nanmean(PAD_profiles_E[res_key][pkeys[pp-5]],axis=1)
        else:
            prof_sp = np.nanmean(PAD_profiles_BNorth[res_key][pkeys[pp-10]],axis=1)

        prof = np.median(prof_sp,axis=0)
        perc = np.percentile(prof_sp,[2.5,97.5],axis=0)
        sp[pp].fill_betweenx(heights[2:],perc[0][2:],perc[1][2:],color=colour[0],alpha=0.3)
        sp[pp].plot(prof[2:],heights[2:],'-',c=colour[0],linewidth=1.5)
...
"""
def plot_profile_sensitivity_to_resolution_individual_CI(figure_name,heights,PAD_profiles_MH,PAD_profiles_wt,PAD_profiles_rad):
...
def plot_profile_sensitivity_to_resolution_individual_CI_single(figure_name,heights,PAD_profiles):
    fig = plt.figure(facecolor='White',figsize=[8,4])
    ax3a = plt.subplot2grid((1,5),(0,0))
    ax3a.set_ylabel('height / m',fontsize=axis_size)
    ax3a.annotate('a - 5 m', xy=(0.05,0.95), xycoords='axes fraction',backgroundcolor='none',horizontalalignment='left', verticalalignment='top', fontsize=10)
    ...
    ax3c = plt.subplot2grid((1,5),(0,2),sharex=ax3a,sharey=ax3a)
    ax3c.annotate('c - 20 m', xy=(0.05,0.95), xycoords='axes fraction',backgroundcolor='none',horizontalalignment='left', verticalalignment='top', fontsize=10)
    ax3c.set_xlabel("relative CIs for individual profiles / %",fontsize=axis_size)
    ...
    pkeys = ['5m','10m','20m','50m','100m']
    for pp in range(0,len(sp)):
        conf2_5 = np.nanpercentile(PAD_profiles[pkeys[pp]]['25'],2.5,axis=0)
        conf25 = np.nanpercentile(PAD_profiles[pkeys[pp]]['25'],25,axis=0)
        conf75 = np.nanpercentile(PAD_profiles[pkeys[pp]]['25'],75,axis=0)
        conf97_5 = np.nanpercentile(PAD_profiles[pkeys[pp]]['25'],97.5,axis=0)
        CI95_i = ((conf97_5-conf2_5)/np.nanmean(PAD_profiles[pkeys[pp]]['25'],axis=0))*100.
        CI50_i = ((conf75-conf25)/np.nanmean(PAD_profiles[pkeys[pp]]['25'],axis=0))*100.
        CI95 = np.nansum(CI95_i,axis=0)/np.sum(np.isfinite(CI95_i),axis=0)
        CI50 = np.nansum(CI50_i,axis=0)/np.sum(np.isfinite(CI50_i),axis=0)
        if pp==2:
            sp[pp].plot(CI95[2:],heights[2:],'-',c=colour[0],linewidth=1,label='50% CI')
            sp[pp].plot(CI50[2:],heights[2:],':',c=colour[0],linewidth=1,label='95% CI')
            lgd = sp[pp].legend(loc=9, bbox_to_anchor=(0.5, -0.3), ncol=2)
        else:
            sp[pp].plot(CI95[2:],heights[2:],':',c=colour[0],linewidth=1)
            sp[pp].plot(CI50[2:],heights[2:],'-',c=colour[0],linewidth=1)
...
"""
Plot individual profile sensitivity to density
"""
...
Plot individual profile sensitivity to resolution
"""
def plot_profile_sensitivity_to_point_density_individual_CI_single(figure_name,heights,PAD_profiles):
    plt.figure(facecolor='White',figsize=[8,4])
    axa = plt.subplot2grid((1,5),(0,0))
    axa.set_ylabel('height / m',fontsize=axis_size)
    axa.annotate('a - 2.5 shots m$^{-2}$', xy=(0.05,0.95), xycoords='axes fraction',
                    backgroundcolor='none',horizontalalignment='left',
                    verticalalignment='top', fontsize=8)
    axb = plt.subplot2grid((1,5),(0,1),sharex=axa,sharey=axa)
    axb.annotate('b - 5 shots m$^{-2}$', xy=(0.05,0.95), xycoords='axes fraction',
                    backgroundcolor='none',horizontalalignment='left',
                    verticalalignment='top', fontsize=8)
    axc = plt.subplot2grid((1,5),(0,2),sharex=axa,sharey=axa)
    axc.annotate('c - 10 shots m$^{-2}$', xy=(0.05,0.95), xycoords='axes fraction',
                    backgroundcolor='none',horizontalalignment='left',
                    verticalalignment='top', fontsize=8)
    axc.set_xlabel("relative CIs for individual profiles / %",fontsize = axis_size)
    axd = plt.subplot2grid((1,5),(0,3),sharex=axa,sharey=axa)
    ...
    pkeys = ['2.5','5','10','20','25']
    for pp in range(0,len(sp)):
        conf2_5 = np.nanpercentile(PAD_profiles['20m'][pkeys[pp]],2.5,axis=0)
        conf25 = np.nanpercentile(PAD_profiles['20m'][pkeys[pp]],25,axis=0)
        conf75 = np.nanpercentile(PAD_profiles['20m'][pkeys[pp]],75,axis=0)
        conf97_5 = np.nanpercentile(PAD_profiles['20m'][pkeys[pp]],97.5,axis=0)
        CI95_i = ((conf97_5-conf2_5)/np.nanmean(PAD_profiles['20m'][pkeys[pp]],axis=0))*100.
        CI50_i = ((conf75-conf25)/np.nanmean(PAD_profiles['20m'][pkeys[pp]],axis=0))*100.
        CI95 = np.nansum(CI95_i,axis=0)/np.sum(np.isfinite(CI95_i),axis=0)
        CI50 = np.nansum(CI50_i,axis=0)/np.sum(np.isfinite(CI50_i),axis=0)
        sp[pp].plot(CI95[2:],heights[2:],':',c=colour[0],linewidth=1)
        sp[pp].plot(CI50[2:],heights[2:],'-',c=colour[0],linewidth=1)
        if pp==2:
            sp[pp].plot(CI50[2:],heights[2:],'-',c=colour[0],linewidth=1,label='50% CI')
            sp[pp].plot(CI95[2:],heights[2:],':',c=colour[0],linewidth=1,label='95% CI')
            lgd = sp[pp].legend(loc=9, bbox_to_anchor=(0.5, -0.3), ncol=2)
        else:
            sp[pp].plot(CI95[2:],heights[2:],':',c=colour[0],linewidth=1)
            sp[pp].plot(CI50[2:],heights[2:],'-',c=colour[0],linewidth=1)
...
def plot_PAI_sensitivity_density(figure_number,figure_name,PAD_profiles_MH_Belian,PAD_profiles_MH_BNorth,PAD_profiles_MH_E,
    ...
    #plt.show()
def plot_PAI_sensitivity_resolution_single(figure_name,PAD_profiles_Belian,PAD_profiles_BNorth,PAD_profiles_E):
    ...
    for rr in range(0,N_res):
        for dd in range(0,N_dens):
            for pp in range(0,len(plot_name)):
                PAD_profiles_Belian[res_keys[rr]][dens_keys[dd]][~np.isfinite(PAD_profiles_Belian[res_keys[rr]][dens_keys[dd]])]=np.nan
                if pp==0:
                    N_obs = np.sum(np.nanmean(PAD_profiles_Belian[res_keys[rr]][dens_keys[dd]],axis=1)[:,2:],axis=1).size
                    PAI = np.append(PAI,np.sum(np.nanmean(PAD_profiles_Belian[res_keys[rr]][dens_keys[dd]],axis=1)[:,2:],axis=1))

                if pp==1:
                    N_obs = np.sum(np.nanmean(PAD_profiles_E[res_keys[rr]][dens_keys[dd]],axis=1)[:,2:],axis=1).size
                    PAI = np.append(PAI,np.sum(np.nanmean(PAD_profiles_E[res_keys[rr]][dens_keys[dd]],axis=1)[:,2:],axis=1))

                if pp==2:
                    N_obs = np.sum(np.nanmean(PAD_profiles_BNorth[res_keys[rr]][dens_keys[dd]],axis=1)[:,2:],axis=1).size
                    PAI = np.append(PAI,np.sum(np.nanmean(PAD_profiles_BNorth[res_keys[rr]][dens_keys[dd]],axis=1)[:,2:],axis=1))
    ...
    df = pd.DataFrame({'plot' : plot,'density' : dens,'resolution' : res,'PAI':PAI})

    fig,axa = plt.subplots(nrows=1,ncols=1, facecolor='White',figsize=(4,3))
    axa.set_ylabel('PAI',fontsize=axis_size)
    sns.violinplot(x='resolution',y='PAI',data=df,linewidth=0.5,inner='box',
                    scale='width',hue="plot",palette = colour,dodge=False,saturation=0.7,cut=0)
    axa.set_xlabel('grid resolution / m')
    axa.set_ylabel('PAI m$^2$/m$^{-1}$',fontsize=axis_size)
    axa.set_ylim(ymin=0)

    axa.yaxis.set_ticks_position('both')
    axa.legend(bbox_to_anchor=(1.2, 0.4, 0.3, 0.2))
    fig.tight_layout()
...
def plot_PAI_sensitivity_density_single(figure_name,PAD_profiles_Belian,
                                        PAD_profiles_BNorth,PAD_profiles_E):

    res_keys = ['20m']; N_res = len(res_keys)
    dens_keys = ['2.5','5','10','20','25']; N_dens = len(dens_keys)
    dens = []; res = []; plot = []
    PAI = np.arange(0)
    plot_name = ['MLA01','SAF03','SAF02']
    for rr in range(0,N_res):
        for dd in range(0,N_dens):
            for pp in range(0,len(plot_name)):
                PAD_profiles_Belian[res_keys[rr]][dens_keys[dd]][~np.isfinite(PAD_profiles_Belian[res_keys[rr]][dens_keys[dd]])]=np.nan
                PAD_profiles_BNorth[res_keys[rr]][dens_keys[dd]][~np.isfinite(PAD_profiles_BNorth[res_keys[rr]][dens_keys[dd]])]=np.nan
                PAD_profiles_E[res_keys[rr]][dens_keys[dd]][~np.isfinite(PAD_profiles_E[res_keys[rr]][dens_keys[dd]])]=np.nan
                if pp==0:
                    PAI = np.append(PAI,np.sum(np.nanmean(PAD_profiles_Belian[res_keys[rr]][dens_keys[dd]],axis=1)[:,2:],axis=1))
                    N_obs = np.sum(np.nanmean(PAD_profiles_Belian[res_keys[rr]][dens_keys[dd]],axis=1)[:,2:],axis=1).size
    ...
    df = pd.DataFrame({'plot' : plot,'density' : dens,'resolution' : res,'PAI':PAI})
    df['density'][df['density']=='5']='05'
    df['density'][df['density']=='2.5']='02.5'

    fig,axa = plt.subplots(nrows=1,ncols=1, facecolor='White',figsize=(4,3))
    axa.set_ylabel('PAI',fontsize=axis_size)
    sns.violinplot(x='density',y='PAI',data=df,linewidth=0.5,inner='box',
                    scale='width',hue="plot",palette = colour,dodge=False,saturation=0.7,cut=0)
    axa.set_xlabel('pulse density / pts m$^{-2}$')
    axa.set_ylabel('PAI m$^2$/m$^{-1}$',fontsize=axis_size)
    axa.set_ylim(ymin=0)
    axa.set_xticklabels(['2.5','5','10','20','25'])
