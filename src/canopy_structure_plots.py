...
def plot_point_cloud(figure_name,figure_number, gps_pts_file,plot_point_cloud):
    ...
    datatype = {'names': ('plot', 'x', 'y', 'x_prime', 'y_prime'), 'formats': ('U32','f16','f16','f16','f16')}
...
def plot_point_clouds_and_profiles_single(figure_name,figure_number, gps_pts_file,plot_point_cloud,
                                heights,lidar_profiles,PAD,PAD_mean,inventory_PAD,inventory_PAD_all):
    n_subplots = PAD['Belian'].shape[0]
    colour = ['#46E900','#1A2BCE','#E0007F']
    rgb = [np.array([[70,233,0]]),np.array([[26,43,206]]),np.array([[224,0,127]])]
    labels = ['$1^{st}$', '$2^{nd}$', '$3^{rd}$', '$4^{th}$']

    heights_rad = np.arange(0,81)
    ...
    plt.figure(figure_number, facecolor='White',figsize=[8,12])

    # Belian
    ax1a = plt.subplot2grid((6,5),(0,0),colspan=2)
    ax1a.annotate('a - Old growth, MLA01', xy=(0.05,0.95), xycoords='axes fraction',backgroundcolor='none',horizontalalignment='left', verticalalignment='top', fontsize=10)
    ax1a.set_ylabel('Height / m',fontsize=axis_size)
    plt.gca().set_aspect('equal', adjustable='box')

    # Seraya
    ax1b = plt.subplot2grid((6,5),(1,0),sharey=ax1a,sharex=ax1a,colspan=2)
    ax1b.annotate('b - Old growth, MLA02', xy=(0.05,0.95), xycoords='axes fraction',backgroundcolor='none',horizontalalignment='left', verticalalignment='top', fontsize=10)
    ax1b.set_ylabel('Height / m',fontsize=axis_size)
    plt.gca().set_aspect('equal', adjustable='box')

    # LF
    ax1c = plt.subplot2grid((6,5),(2,0),sharey=ax1a,sharex=ax1a,colspan=2)
    ax1c.annotate('c - Moderately logged, SAF04', xy=(0.05,0.95), xycoords='axes fraction',backgroundcolor='none',horizontalalignment='left', verticalalignment='top', fontsize=10)
    ax1c.set_ylabel('Height / m',fontsize=axis_size)
    plt.gca().set_aspect('equal', adjustable='box')

    # E
    ax1d = plt.subplot2grid((6,5),(3,0),sharey=ax1a,sharex=ax1a,colspan=2)
    ax1d.annotate('d - Moderately logged, SAF05', xy=(0.05,0.95), xycoords='axes fraction',backgroundcolor='none',horizontalalignment='left', verticalalignment='top', fontsize=10)
    ax1d.set_ylabel('Height / m',fontsize=axis_size)
    plt.gca().set_aspect('equal', adjustable='box')

    # B North
    ax1e = plt.subplot2grid((6,5),(4,0),sharey=ax1a,sharex=ax1a,colspan=2)
    ax1e.annotate('e - Heavily logged, SAF02', xy=(0.05,0.95), xycoords='axes fraction',backgroundcolor='none',horizontalalignment='left', verticalalignment='top', fontsize=10)
    ax1e.set_ylabel('Height / m',fontsize=axis_size)
    plt.gca().set_aspect('equal', adjustable='box')

    # B South
    ax1f = plt.subplot2grid((6,5),(5,0),sharey=ax1a,sharex=ax1a,colspan=2)
    ax1f.annotate('f - Heavily logged, SAF01', xy=(0.05,0.95), xycoords='axes fraction',backgroundcolor='none',horizontalalignment='left', verticalalignment='top', fontsize=10)
    ax1f.set_ylabel('Height / m',fontsize=axis_size)
    ax1f.set_xlabel('Horizontal distance / m',fontsize=axis_size)
    plt.gca().set_aspect('equal', adjustable='box')
    ...
    # NOW PLOT PROFILES
    # Belian
    ax2a = plt.subplot2grid((6,5),(0,2),sharey=ax1a)
    ax2a.annotate('LiDAR returns', xy=(0.95,0.95), xycoords='axes fraction',backgroundcolor='none',horizontalalignment='right', verticalalignment='top', fontsize=9)
    # - PAD
    ax3a = plt.subplot2grid((6,5),(0,3),sharey=ax1a)
    ax3a.annotate('PAD profile', xy=(0.95,0.95), xycoords='axes fraction',backgroundcolor='none',horizontalalignment='right', verticalalignment='top', fontsize=9)

    # Crown VOlume
    ax4a = plt.subplot2grid((6,5),(0,4),sharey=ax1a,sharex=ax3a)
    ax4a.annotate('crown volume', xy=(0.95,0.95), xycoords='axes fraction',backgroundcolor='none',horizontalalignment='right', verticalalignment='top', fontsize=9)

    # Seraya
    ax2b = plt.subplot2grid((6,5),(1,2),sharey=ax1a,sharex=ax2a)
    ax3b = plt.subplot2grid((6,5),(1,3),sharey=ax1a,sharex=ax3a)
    ax4b = plt.subplot2grid((6,5),(1,4),sharey=ax1a,sharex=ax3a)

    # LF
    ax2c = plt.subplot2grid((6,5),(2,2),sharey=ax1a,sharex=ax2a)
    ax3c = plt.subplot2grid((6,5),(2,3),sharey=ax1a,sharex=ax3a)
    ax4c = plt.subplot2grid((6,5),(2,4),sharey=ax1a,sharex=ax3a)

    # E
    ax2d = plt.subplot2grid((6,5),(3,2),sharey=ax1a,sharex=ax2a)
    ax3d = plt.subplot2grid((6,5),(3,3),sharey=ax1a,sharex=ax3a)
    ax4d = plt.subplot2grid((6,5),(3,4),sharey=ax1a,sharex=ax3a)

    # B North
    ax2e = plt.subplot2grid((6,5),(4,2),sharey=ax1a,sharex=ax2a)
    ax3e = plt.subplot2grid((6,5),(4,3),sharey=ax1a,sharex=ax3a)
    ax4e = plt.subplot2grid((6,5),(4,4),sharey=ax1a,sharex=ax3a)

    # B South
    ax2f = plt.subplot2grid((6,5),(5,2), sharex = ax1a, sharey = ax2a)
    ax2f.set_xlabel('Number of returns\n(x1000)',fontsize=axis_size,horizontalalignment='center')
    # - MacHorn
    ax3f = plt.subplot2grid((6,5),(5,3),sharey=ax1a, sharex=ax3a)
    ax3f.set_xlabel('PAD\n(m$^2$m$^{-2}$m$^{-1}$)',fontsize=axis_size,horizontalalignment='center')
    # - Inventory
    ax4f = plt.subplot2grid((6,5),(5,4),sharey=ax1a,sharex=ax3a)
    ax4f.set_xlabel('Crown Volume\n(m$^3$m$^{-2}$m$^{-1}$)',fontsize=axis_size,horizontalalignment='center')
    ...
    for pp in range(0,6):
        ...
        # plot LIDAR profiles
        for i in range(0,n_subplots):
            axes2[pp].fill_betweenx(heights[2:],0,PAD[Plot_name][i,2:],color=colour[0],alpha=0.04)
        axes2[pp].plot(PAD_mean[Plot_name][2:],heights[2:],'-',c=colour[0],linewidth=2)
        # inventory
        llim,ulim = np.percentile(inventory_PAD_all[Plot_name],[2.5,97.5],axis =0)
        axes3[pp].fill_betweenx(heights[2:],llim[2:],ulim[2:],color='black',alpha=0.5)
        axes3[pp].plot(inventory_PAD[Plot_name][2:],heights[2:],'-',c='black',linewidth=2)
    ...
    ax1a.set_xlim(0,100); ax1a.set_ylim(0,80)
    ax2a.set_xlim(0,29);  ax3a.set_xlim(left=0,right=0.9)

    for ax in [ax2f,ax3f,ax4f]:
        ax.locator_params(axis='x',nbins=5)
...
def plot_point_clouds_and_profiles_Danum_single(figure_name,figure_number, gps_pts_file,
                                plot_point_cloud,heights,lidar_profiles,
                                PAD,PAD_mean,inventory_PAD,inventory_PAD_all):
    max_return=3
    n_subplots = 25
    heights_plus_ground = np.arange(0,np.max(heights)+1)
    colour = ['#46E900','#1A2BCE','#E0007F']
    rgb = [np.array([[70,233,0]]),np.array([[26,43,206]]),np.array([[224,0,127]])]
    labels = ['$1^{st}$', '$2^{nd}$', '$3^{rd}$', '$4^{th}$']
    ...
    # DC1
    ax1a = plt.subplot2grid((2,5),(0,0),colspan=2)
    ax1a.annotate('a - Old growth, DAN04', xy=(0.05,0.95), xycoords='axes fraction',backgroundcolor='none',horizontalalignment='left', verticalalignment='top', fontsize=10)
    ax1a.set_ylabel('Height / m',fontsize=axis_size)
    plt.gca().set_aspect('equal', adjustable='box')

    # DC2
    ax1b = plt.subplot2grid((2,5),(1,0),sharey=ax1a,sharex=ax1a,colspan=2)
    ax1b.annotate('b - Old growth, DAN05', xy=(0.05,0.95), xycoords='axes fraction',backgroundcolor='none',horizontalalignment='left', verticalalignment='top', fontsize=10)
    ax1b.set_ylabel('Height / m',fontsize=axis_size)
    plt.gca().set_aspect('equal', adjustable='box')
    ...
    # NOW PLOT PROFILES
    # DC1
    ax2a = plt.subplot2grid((2,5),(0,2),sharey=ax1a)
    ax2a.annotate('LiDAR returns', xy=(0.95,0.95), xycoords='axes fraction',backgroundcolor='none',horizontalalignment='right', verticalalignment='top', fontsize=9)
    # - MacHorn
    ax3a = plt.subplot2grid((2,5),(0,3),sharey=ax1a)
    ax3a.annotate('PAD profile', xy=(0.95,0.95), xycoords='axes fraction',backgroundcolor='none',horizontalalignment='right', verticalalignment='top', fontsize=9)
    # - crown volume
    ax4a = plt.subplot2grid((2,5),(0,4),sharey=ax1a,sharex=ax3a)
    ax4a.annotate('crown volume', xy=(0.95,0.95), xycoords='axes fraction',backgroundcolor='none',horizontalalignment='right', verticalalignment='top', fontsize=9)

    # Danum 2
    ax2b = plt.subplot2grid((2,5),(1,2), sharex = ax2a, sharey = ax1a)
    ax2b.set_xlabel('Number of returns',fontsize=axis_size,horizontalalignment='center')
    # - MacHorn
    ax3b = plt.subplot2grid((2,5),(1,3),sharey=ax1a, sharex=ax3a)
    ax3b.set_xlabel('PAD\n(m$^2$m$^{-2}$m$^{-1}$)',fontsize=axis_size,horizontalalignment='center')
    # - Inventory
    ax4b = plt.subplot2grid((2,5),(1,4),sharey=ax1a,sharex=ax3a)
    ax4b.set_xlabel('Crown Volume\n(m$^3$m$^{-2}$m$^{-1}$)',fontsize=axis_size,horizontalalignment='center')
    ...
    for pp in range(0,2):
        ...
        return_dist     = np.sum(lidar_profiles[Plot_name],axis=0)
        for k in range(0,max_return):
            axes1[pp].plot(return_dist[:,k],np.max(heights_plus_ground)-heights_plus_ground,'-',c=colour[k],linewidth=1)
    ...
    for ax in [ax2b,ax3b,ax4b]:
        ax.locator_params(axis='x',nbins=5)

    ax1a.set_xlim(0,100); ax1a.set_ylim(0,80)
    ax2a.set_xlim(0,3500); ax3a.set_xlim(left=0,right=0.9)
...
def plot_PAI_vs_basal_area_single(figure_name,figure_number,PAD,PAD_mean,BasalArea,
                            plot_marker,plot_label,plot_colour,layer_thickness=1):

    Plots = list(PAD.keys())
    N_plots = len(Plots)
    N_subplots = np.shape(PAD[Plots[0]])[0]
    BA = np.zeros(N_plots)
    BAerr = np.zeros(N_plots)
    PAI = np.zeros(N_plots)
    PAIerr = np.zeros(N_plots)
    order = [2,3,6,7,0,1,4,5]
    for pp in range(0,N_plots):
        print(Plots[order[pp]])
        Plots_str = Plots[order[pp]]
        # for 1 ha averages, want to avoid nodata
        # these should be identifiable based on penetration limit
        BA[pp] = BasalArea[Plots_str][0]
        BAerr[pp] = BasalArea[Plots_str][1]
        PAI[pp] = np.nansum(PAD_mean[Plots[order[pp]]])*layer_thickness
        PAIerr[pp] = stats.sem(np.nansum(PAD[Plots[order[pp]]],axis=1)*layer_thickness)

    # annotate with stats
    r_sq =   aux.get_rsquared_annotation(BA,PAI)

    plt.figure(figure_number, facecolor='White',figsize=[4,5])
    ax1 = plt.subplot2grid((1,1),(0,0))
    ax1.set_xlabel('Basal Area / m$^2$ha$^{-1}$',fontsize=axis_size)
    ax1.set_ylabel('PAI',fontsize=axis_size)
    ax1.annotate(r_sq, xy=(0.95,0.05), xycoords='axes fraction',backgroundcolor='none',horizontalalignment='right', verticalalignment='bottom', fontsize=axis_size)

    # plot linear regression
    m, c, r2, p, x_i, y_i, CI_u, CI_l, PI_u, PI_l = field.linear_regression(BA,PAI,conf=0.95)
    ax1.fill_between(x_i,CI_l,CI_u,color='0.75')
    ax1.plot(x_i,y_i,'-',color='black')
    for i in range(0,N_plots):
        Plot_str = Plots[order[i]]
        ax1.errorbar(BA[i],PAI[i],xerr=BAerr[i],yerr=PAIerr[i],marker='None',
                    color=plot_colour[Plot_str],alpha=0.5)
        ax1.plot(BA[i],PAI[i],marker=plot_marker[Plot_str],
                    color=plot_colour[Plot_str],linestyle='None',label=plot_label[Plot_str])

    lgd = ax1.legend(loc=10, bbox_to_anchor=(0.5, -0.25), ncol=4)
...
def plot_location_map(figure_name,figure_number):
    ...
    AGB_file = '/home/dmilodow/DataStore_DTM/BALI/BALI_maps/CAO_Sabah_Carbon_Map_100m_average_WGS84.tif'
    ref_file = '/home/dmilodow/DataStore_GCEL/AGB/avitabile/Avitabile_AGB_Map/Avitabile_AGB_Map.tif'

    ds = gdal.Open(AGB_file)
    geoTrans = ds.GetGeoTransform()
    W = 116.; E = 119.4; N = 7; S = 4.1
    #agb,geoT=geo.clip_array_to_bbox(np.asarray(ds.ReadAsArray(),dtype='float'),geoTrans,N,S,W,E)
    agb = xr.open_rasterio(AGB_file).sel(band=1).sel(x=slice(W,E),y=slice(N,S))
    ref = xr.open_rasterio(ref_file).sel(band=1).sel(x=slice(W,E),y=slice(N,S))

    agb.values[agb.values<0]=np.nan; agb_array = ma.masked_invalid(agb.values)
    ref.values[ref.values<0]=np.nan; ref.values[np.isfinite(ref.values)] = 1; ref_array = ma.masked_invalid(ref.values)
    ...
    for b in shp.records():
        if b.attributes['admin']=='Malaysia':
            sp = ShapelyFeature(b.geometry, crs,  facecolor='0.5', edgecolor = 'none')
            ax.add_feature(sp,zorder = 2)
        elif b.attributes['admin']=='Indonesia':
            sp = ShapelyFeature(b.geometry, crs,  facecolor = '0.5', edgecolor='none')
            ax.add_feature(sp, zorder = 1)
    """
    imref = plt.pcolormesh(ref.x.values,ref.y.values, ref_array, cmap='Greys', vmin=0,vmax=2, zorder=2)
    im = plt.pcolormesh(agb.x.values,agb.y.values, agb_array, cmap='Greens', zorder=3)
    ...
    for pp in pshp.records():
        ...
        if pp.attributes['ForestType'] == 'OG':
            ...
            col=colour[2]
        ax.plot(pp.geometry.coords.xy[0],pp.geometry.coords.xy[1], marker='o', mfc=col,mec='black', mew=0.5, markersize=6,zorder=4)

    ax.set_xlim(left=W,right=E);ax.set_ylim(top=N,bottom=S)


    # Grid lines
    parallels = np.arange(0.,12.,1.); meridians = np.arange(110.,122.,1.)

    gl = ax.gridlines(crs=ccrs.PlateCarree(), draw_labels=True,
                      linewidth=1, color='gray', linestyle='--',zorder=10)
    ...
    gl.xlabel_style = {'color': '0.5'}; gl.ylabel_style = {'color': '0.75'}
    ...
    cbar.ax.set_ylabel('Aboveground Carbon Density / Mg C ha$^{-1}$',fontsize = 15)
    ...
    ax.annotate('BRUNEI', xy=(114.4,4.74), xycoords='data',
            backgroundcolor='none',horizontalalignment='center', verticalalignment='center',
            fontsize=12, style='italic')
    ax.annotate('N. KALIMANTAN', xy=(116.5,4.2), xycoords='data',
            backgroundcolor='none',horizontalalignment='center', verticalalignment='center',
            fontsize=12, style='italic')
    ax.annotate('SAFE', xy=(117.63,4.7), xycoords='data',backgroundcolor='none',
...
def plot_PAD_distributions_for_canopy_subdivisions(figure_name,figure_number,
    ...
    plot_name_1 = ['Belian','Seraya','DC1','DC2','LF','E','B North','B South']
...
def plot_cumulative_PAD_vs_depth(figure_name,figure_number,MacArthurHorn_PAD,
    ...
    max_return=2
    fig_plots = [['Belian','Seraya'],['DC1','DC2'],['LF','E'],['B North','B South']]
...
def plot_PAI_Shannon_Index_distributions(figure_name,figure_number,PAD):
    ...
    for pp,plot in enumerate(PAD.keys()):
        ...
        if plot in ['Belian','Seraya']:
            region += ['Maliau\nBasin']*n_subplots
            forest_type += ['OG']*n_subplots
        elif plot in ['DC1','DC2']:
            region += ['Danum\nValley']*n_subplots
            forest_type += ['OG']*n_subplots
        elif plot in ['LF','E']:
            region += ['SAFE\nmoderately\nlogged']*n_subplots
...
def plot_shannon_diversity_index_distributions(figure_name,PAD,heights):

    palette_colour = ['#30A000','#46E900','#1A2BCE','#E0007F']
    fig = plt.figure(facecolor='White',figsize=(4,3))
...
def plot_subcanopy_environments(figure_name,PAD,heights):
    n_plots = len(PAD.keys())
    vertical_resolution = 1
    n_layers = heights.size-2
    cumulative_PAD = np.zeros(n_subplots*n_plots*n_layers)
